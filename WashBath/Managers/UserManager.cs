...
namespace WashBath.Managers
{
    ...
    public class UserManager<TUser> : Microsoft.AspNet.Identity.UserManager<TUser> where TUser : SaleUser
    {
        ...
        public UserManager(SaleUserStore<TUser> store)
          : base(store)
        {
            m_MemoryCache = CachingHelper.GetMemoryCache("WashBathUser");
        }

        public async Task<SaleUser> FindUser(string userName, string password)
        {
        ...
        {
            return await Users.Where(u => u.Status == UserStatus.Normal && u.Role.Code == roleCode).ToListAsync();
        }
